...
class SpotifyApiService {
  ...
  async fadeOutAndPause(durationMs: number = 2000) {
    try {
      ...
      // Pause and restore original volume
      await this.pause();
      await this.setVolume(startVolume);
    } catch (error) {
  ...
  async playAndFadeIn(durationMs: number = 1000, deviceId?: string) {
    try {
      ...
      const targetVolume = state.device?.volume_percent || 100;
      ...
      try {
        ...
        // If volume control fails, just play normally
